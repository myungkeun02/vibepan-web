interface Props {
  name: string;
}
import type { ReactNode } from 'react';

export default function UiIcon(props: Props & { children?: ReactNode }) {
  const paths: Record<string, string> = {
    search: 'm21 21-4.5-4.5M19 10.5a8.5 8.5 0 1 1-17 0 8.5 8.5 0 0 1 17 0',
    grid: 'M3 3h7v7H3zM14 3h7v7h-7zM3 14h7v7H3zM14 14h7v7h-7z',
    chat: 'M21 11.5a8.5 8.5 0 0 1-8.5 8.5H4l-3 2V11.5a10 10 0 0 1 20 0ZM7 10h8M7 14h5',
    chart: 'M4 20V10M12 20V4M20 20v-7',
    user: 'M20 21v-2a6 6 0 0 0-6-6h-4a6 6 0 0 0-6 6v2M16 5a4 4 0 1 1-8 0 4 4 0 0 1 8 0',
    back: 'm14 5-7 7 7 7',
    bell: 'M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4',
    filter: 'M3 6h18M3 18h18M7 3v6M17 15v6',
    close: 'm6 6 12 12M6 18 18 6',
    plus: 'M12 5v14M5 12h14',
    arrow: 'M5 12h14m-6-6 6 6-6 6',
  };
  return (
    <>
      <svg
        width={'22'}
        height={'22'}
        viewBox={'0 0 24 24'}
        fill={'none'}
        stroke={'currentColor'}
        strokeWidth={'1.4'}
        strokeLinecap={'round'}
        strokeLinejoin={'round'}
        aria-hidden={'true'}
      >
        <path d={paths[props.name] || paths.grid} />
      </svg>
    </>
  );
}
